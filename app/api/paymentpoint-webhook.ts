...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!hasValidPaymentPointSignature(rawBody, signature, paymentPointSecretKey)) {
      logPayment({ event: 'paymentpoint-webhook-invalid-signature', scope: 'app-api' });
    ...
    if (!validation.ok) {
      if (validation.ignore) {
        logPayment({ event: 'paymentpoint-webhook-ignored', scope: 'app-api', message: validation.message });
    ...
    logError('paymentpoint-webhook-unexpected-error', error, { scope: 'app-api' });
